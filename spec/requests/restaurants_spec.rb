...
RSpec.describe "Restaurants", type: :request do
 ...
 describe "GET /new restaurants" do
    ...
    it "Signing in as admin" do
      user=FactoryBot.create(:user,email:"check@gmail.com",password:"Abhi",password_confirmation:"Abhi",isadmin: true)
      post sessions_url, params:{email: user.email, password: user.password}
      get '/restaurants/new'
      expect(response).to have_http_status(200)
    end
  ...
  describe "PATCH /update restaurant information" do
    it "update restaurant information" do
      restaurant= create(:restaurant)
      patch '/restaurants/'+restaurant.id.to_s, params:{restaurant:{name:"Updated name",category:"Updated",resturant_type:"Updated"}}
      expect(response).to have_http_status(302)
    end
  end 
